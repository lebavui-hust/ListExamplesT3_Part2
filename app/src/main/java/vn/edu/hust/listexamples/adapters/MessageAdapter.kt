...
class MessageAdapter(val messages: List<MessageModel>): BaseAdapter() {
  ...
  override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
    ...
    if (convertView == null) {
      if (getItemViewType(position) == 0)
        itemView = LayoutInflater.from(parent?.context).inflate(R.layout.layout_item_left, parent, false)
      else
        itemView = LayoutInflater.from(parent?.context).inflate(R.layout.layout_item_right, parent, false)
  ...
  class ViewHolder(itemView: View) {
    ...
    val textMessage: TextView = itemView.findViewById(R.id.text_title)
  }
}